...
class ModelTrainer(nn.Module):
    ...
    def compute_loss(self, pred_lf_chunk, targ_chunk, model_output_seg, targ_seg_chunk, targ_prior_chunk, targ_hf_chunk, model_output_seg_pre, model_output_img):
    ...
    def train_step(self, losses, coord_input_loader, model: nn.Module, target_gt: torch.Tensor, target_seg: torch.Tensor, target_prior: torch.Tensor, target_hf: torch.Tensor):
        ...
        for idx, patch_grid in (enumerate(coord_input_loader)):
            ...
            loss, L_mse, L_prior, L_seg, TV_seg, TV_img, mse_hf = self.compute_loss(pred_lf_chunk, targ_chunk, model_output_seg, targ_seg_chunk, targ_prior_chunk, targ_hf_chunk, model_output_seg_pre, model_output_img) #total_loss per chunk
            loss.backward() 
    ...
    def train_inr(self):
        ...
        target_hf = hf_ground_truth[:,1:-1,:].to(device) #shape: [192, 172, 192]
        ...
        for ep in tqdm(range(self.epochs)):
            ...
            model, total_loss, mse_per_epoch, seg_per_epoch, prior_per_epoch, tv_seg_per_epoch, tv_img_per_epoch = self.train_step(losses, coord_input_loader, model, target_gt, target_seg, target_prior, target_hf)
