...
from Models.model import MLP, initialize_siren_weights, SineLayer, ReLULayer, WIRELayer, initialize_wire_weights, FourierFeatures
...
# def main():
    ...
    if(activation_fn == 'SIREN'):
        inr_mlp = MLP(in_size=ff_pos_enc.out_size, out_size=5, hidden_size=HIDDEN_SIZE, num_layers=NUM_LAYERS, layer_class=SineLayer, siren_factor=SIREN_FACTOR,)
        initialize_siren_weights(inr_mlp, SIREN_FACTOR) # Re-initialize the weights and make sure they are different                                
    else:    
        inr_mlp = MLP(in_size=ff_pos_enc.out_size, out_size=5, hidden_size=HIDDEN_SIZE, num_layers=NUM_LAYERS, layer_class=WIRELayer,  wire_omega=WIRE_OMEGA, wire_sigma=WIRE_SIGMA)
    ...
    trainer_module = PosEncINRLightningModule(pos_encoder=ff_pos_enc,
                                            network=inr_mlp,
                                            lr=LEARNING_RATE,
