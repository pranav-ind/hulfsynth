...
from projects.hulfsynth.hulfsynth.sample_codes.ImagePreparation import ImagePreparation
...
def priors(device, dataset_num):
    ...
    wm_prior = torch.from_numpy(nib.load(wm_location).get_fdata()[:,:,95])
    gm_prior = torch.from_numpy(nib.load(gm_location).get_fdata()[:,:,95])
    csf_prior = torch.from_numpy(nib.load(csf_location).get_fdata()[:,:,95])
...
def lf_segmentations(device, dataset_num):
    ...
    lf_wm_seg = torch.from_numpy(nib.load(lf_wm_location).get_fdata()[:,:,95])
    lf_gm_seg = torch.from_numpy(nib.load(lf_gm_location).get_fdata()[:,:,95])
    lf_csf_seg = torch.from_numpy(nib.load(lf_csf_location).get_fdata()[:,:,95])
...
def load_data_sensitivity_analysis(dataset_num, config=default_config):
    ...
    hf_ground_truth = nib.load(hf_loc).get_fdata()[:,:,slice] 
    ...
    config["size"] = (hf_ground_truth.shape[0], hf_ground_truth.shape[1]) #Loading 2D Images. For datasets = 1, 2 (174, 192)
    config["size_lf"] = (lf_gt.shape[0], lf_gt.shape[1])
    # config["M"] = M
    ...
    return hf_ground_truth, lf_like, prior_seg_dice, lf_gt_seg_dice, M
...
def load_data(dataset_num, config=default_config):
    ...
    return hf_ground_truth, lf_dataloader, prior_seg_dice, lf_gt_seg_dice, M
