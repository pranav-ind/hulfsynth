'''
- This file contains functions that estimate contrast factor (for validation dataset)
- Merged load_val() from Data>load_val.py to this file
...
def seg_to_intenities(img_nib, wm_nib, gm_nib, csf_nib, bg):
    '''
    - This function maps tissue segmentations to intensities
...
def get_target_c(dataset_num='0011', target_type='hf'):
    _, roi_snrs = get_rois(dataset_num, field_type=target_type) #returns HF SNRs if target_type == HF
    S_list = roi_snrs
...
def get_m(dataset_num='0011', target_type='hf'):
    ...
    # target_c = np.array([2, 12, 7]) #fixed target contrast
    target_c = get_target_c(dataset_num, target_type) #target contrast based on ULF SNRs
...
class GridSearch :
  ...
  def select_solution(self, grid_results):
    ...
    if(len(temp_df[temp_df['log_constraint']<1]) > 0):
        ...
        print("Choosing least cost function because log constraint did not find optimal solution; The solutions might have different order of magnitude")
...
def forward(dataset_num='0011', target_type='ulf'):
    '''
    This function will return observed HF, LF and computed M
    '''
...
def load_val_data(target_type, config=default_config):
    #modified this function here to be included in this file from the earlier version: Data > load_val.py
    slice = config["slice"] 
    dataset_num = config["dataset_num"]
...
def calc_val_ulf_snr(img_nib, wm_nib, gm_nib, csf_nib, dataset):
    #test function for a fixed single subject
    rayleigh_correction = 1.53
    if(dataset == 'val'):
